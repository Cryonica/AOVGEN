...
namespace AOVGEN
{
    public partial class CableSettings : RadForm
    {
            ...
            using (SQLiteConnection connection = OpenDB())
            {
                ...
                if (connection.State == ConnectionState.Open)
                {
                    ...
                    using (SQLiteDataReader reader2 = command.ExecuteReader())
                    {
                        ...
                        if (reader2.HasRows)
                        {
                            string cabD, cabC, cabP, cabDCS, cabCCS, cabPCS, devider, cablename, cabApipe, cabDpipe, cabCpipe, cabPpipe, cabAtray, cabDtray, cabCtray, cabPtray, buildpreset, writecableP, writepumpcable, writevalvecable;
                            var cabA = cabD = cabC = cabP = cabDCS = cabCCS = cabPCS = devider = cablename = cabApipe = cabDpipe = cabCpipe = cabPpipe = cabAtray = cabDtray = cabCtray = cabPtray = buildpreset = writecableP = writepumpcable = writevalvecable = string.Empty;
                            while (reader2.Read())
                            {
                                //reader2[0] - ID
        ...
        private void radToggleSwitch1_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (radCheckedDropDownList5.Visible) return;
